...
-- Dynamic backlog: seed from OPTIMIZATION-TARGETS.md, enriched by workers/researcher.

CREATE TABLE IF NOT EXISTS optimization_targets (
    id              TEXT PRIMARY KEY,       -- "EVM-1", "TRIE-5", etc.
    area            TEXT NOT NULL,          -- "evm", "trie", "state", "rlp", "db", "bp"
    title           TEXT NOT NULL,
    description     TEXT,                   -- full markdown from targets file
    difficulty      TEXT CHECK (difficulty IN ('S','M','L')),
    impact          TEXT CHECK (impact IN ('low','med','high')),
    priority_score  REAL DEFAULT 0,         -- impact_w * ease_w
    ...
    proposed_by     TEXT,
    related_targets TEXT,                   -- JSON array
